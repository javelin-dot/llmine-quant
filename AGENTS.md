...
`src/data/emptyScreens.ts` defines empty shape fallbacks when `/api/v1` is unavailable. When you extend `MockData` in `types.ts`, update `emptyScreens.ts` so fallback payloads stay type-correct.
