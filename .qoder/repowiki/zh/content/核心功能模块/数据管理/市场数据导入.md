# 市场数据导入

<cite>
**本文引用的文件**
- [backend/app/services/market_data_import.py](file://backend/app/services/market_data_import.py)
- [backend/app/services/market_data_full_sync.py](file://backend/app/services/market_data_full_sync.py)
- [backend/app/services/_market_fetch_worker.py](file://backend/app/services/_market_fetch_worker.py)
- [backend/scripts/import_market_data.py](file://backend/scripts/import_market_data.py)
- [backend/app/integrations/market_data/base.py](file://backend/app/integrations/market_data/base.py)
- [backend/app/integrations/market_data/factory.py](file://backend/app/integrations/market_data/factory.py)
- [backend/app/integrations/market_data/akshare_provider.py](file://backend/app/integrations/market_data/akshare_provider.py)
- [backend/app/integrations/market_data/mock_provider.py](file://backend/app/integrations/market_data/mock_provider.py)
- [backend/app/integrations/market_data/adapter.py](file://backend/app/integrations/market_data/adapter.py)
- [backend/app/api/v1/data.py](file://backend/app/api/v1/data.py)
- [backend/app/domains/data/models.py](file://backend/app/domains/data/models.py)
- [backend/app/domains/data/schemas.py](file://backend/app/domains/data/schemas.py)
- [backend/tests/services/test_market_data_import.py](file://backend/tests/services/test_market_data_import.py)
</cite>

## 更新摘要
**变更内容**
- 新增全市场同步服务：添加 `market_data_full_sync.py` 实现完整的A股市场数据同步
- 新增进程池并发处理：通过 `_market_fetch_worker.py` 实现高性能的数据获取
- 新增API接口：添加 `/market-bars/sync/full` 触发全量同步和 `/market-bars/sync/status` 进度查询
- 新增数据模型：扩展 `StockInfo` 模型支持股票基本信息存储
- 新增前端集成：前端组件支持全市场同步的可视化操作

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本技术文档聚焦"市场数据导入"能力，覆盖以下导入方式与实现要点：
- CSV 文件导入：本地 CSV 导入服务，字段别名映射、标准化与清洗、去重与更新替换、A 股特殊标志计算。
- AKShare 数据源导入：通过 AKShare 获取 A 股日线行情，异步封装与并发拉取。
- **全市场同步服务**：新增的完整A股市场数据同步功能，支持全量和增量更新、进程池并发处理、ST标记更新。
- Tushare 数据源导入：通过工厂模式接入 Tushare Provider（适配器接口预留，具体实现按需扩展）。
- 架构设计：服务层、提供者抽象、工厂选择、API 屏蔽层、数据库持久化。
- 批量处理机制：内存中先归一化、去重、校验，再批量插入或替换。
- 错误处理策略：字段缺失、数值非法、重复键、网络/安装异常等。
- 性能优化方案：异步线程池、批量提交、唯一索引约束、字段别名与宽松解析。

## 项目结构
围绕"市场数据导入"的相关模块组织如下：
- 服务层：导入服务与提供者实现
- **全市场同步服务**：完整的A股市场数据同步与管理
- 集成层：市场数据提供者抽象与工厂
- API 层：导入接口与响应封装
- 数据模型：数据库表定义与唯一约束
- 脚本：命令行导入入口
- 测试：导入服务行为验证

```mermaid
graph TB
subgraph "导入服务层"
Svc["MarketDataImportService<br/>导入主流程"]
CSVProv["LocalCsvDailyBarProvider<br/>CSV读取"]
AKProv["AKShareDailyBarProvider<br/>AKShare抓取"]
end
subgraph "全市场同步服务层"
FullSync["MarketDataFullSyncService<br/>全量/增量同步"]
Worker["MarketFetchWorker<br/>进程池数据获取"]
end
subgraph "集成层"
Base["MarketDataProvider<br/>抽象接口"]
Factory["get_market_data_provider()<br/>工厂选择"]
AKShare["AKShareProvider<br/>实时/历史提供者"]
Mock["MockMarketDataProvider<br/>模拟提供者"]
end
subgraph "API 层"
API["/market-bars/import/*<br/>导入接口"]
FullSyncAPI["/market-bars/sync/full<br/>全量同步接口"]
StatusAPI["/market-bars/sync/status<br/>进度查询接口"]
end
subgraph "数据模型"
Model["MarketBarDaily<br/>唯一约束: symbol+trade_date"]
StockInfo["StockInfo<br/>股票基本信息"]
end
Script["命令行导入脚本"]
CSVProv --> Svc
AKProv --> Svc
Svc --> Model
FullSync --> Worker
FullSync --> Model
FullSync --> StockInfo
API --> Svc
FullSyncAPI --> FullSync
StatusAPI --> FullSync
Script --> Svc
Factory --> AKShare
Factory --> Mock
AKShare --> |"get_history()/get_snapshot()"| API
```

**图表来源**
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_full_sync.py:233-246](file://backend/app/services/market_data_full_sync.py#L233-L246)
- [backend/app/services/_market_fetch_worker.py:20-44](file://backend/app/services/_market_fetch_worker.py#L20-L44)
- [backend/app/integrations/market_data/base.py:64-113](file://backend/app/integrations/market_data/base.py#L64-L113)
- [backend/app/integrations/market_data/factory.py:23-57](file://backend/app/integrations/market_data/factory.py#L23-L57)
- [backend/app/integrations/market_data/akshare_provider.py:145-262](file://backend/app/integrations/market_data/akshare_provider.py#L145-L262)
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)
- [backend/app/domains/data/models.py:40-86](file://backend/app/domains/data/models.py#L40-L86)

**章节来源**
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_full_sync.py:1-416](file://backend/app/services/market_data_full_sync.py#L1-L416)
- [backend/app/services/_market_fetch_worker.py:1-44](file://backend/app/services/_market_fetch_worker.py#L1-L44)
- [backend/app/integrations/market_data/base.py:64-113](file://backend/app/integrations/market_data/base.py#L64-L113)
- [backend/app/integrations/market_data/factory.py:23-57](file://backend/app/integrations/market_data/factory.py#L23-L57)
- [backend/app/integrations/market_data/akshare_provider.py:145-262](file://backend/app/integrations/market_data/akshare_provider.py#L145-L262)
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)
- [backend/app/domains/data/models.py:40-86](file://backend/app/domains/data/models.py#L40-L86)

## 核心组件
- 导入服务
  - 职责：统一接收原始行、标准化/清洗/去重、A 股标志计算、替换旧记录、批量持久化。
  - 关键点：字段别名映射、严格校验、重复键去重、批量删除旧记录后再新增。
- **全市场同步服务**
  - 职责：扫描全市场股票、计算增量起点、进程池并发获取、批量持久化、ST标记更新。
  - 关键点：进程池隔离、失败重试、进度跟踪、状态管理。
- CSV 提供者
  - 职责：读取本地 UTF-8 CSV 文件，返回字典行列表。
  - 异常：文件不存在、路径非文件。
- AKShare 提供者
  - 职责：异步并发抓取多个股票的日线行情，转换为统一结构。
  - 异常：未安装包时提示改用本地导入。
- 历史/快照提供者（集成层）
  - 职责：抽象历史与快照接口；工厂根据配置选择 AKShare/Tushare/Mock。
- 数据模型
  - 职责：定义每日行情表结构与唯一约束（symbol+trade_date），便于幂等写入。
  - **新增**：StockInfo模型支持股票基本信息存储和显示。

**章节来源**
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_full_sync.py:233-246](file://backend/app/services/market_data_full_sync.py#L233-L246)
- [backend/app/services/market_data_full_sync.py:105-121](file://backend/app/services/market_data_full_sync.py#L105-L121)
- [backend/app/services/market_data_full_sync.py:146-192](file://backend/app/services/market_data_full_sync.py#L146-L192)
- [backend/app/services/market_data_import.py:107-120](file://backend/app/services/market_data_import.py#L107-L120)
- [backend/app/services/market_data_import.py:122-175](file://backend/app/services/market_data_import.py#L122-L175)
- [backend/app/integrations/market_data/base.py:64-113](file://backend/app/integrations/market_data/base.py#L64-L113)
- [backend/app/integrations/market_data/factory.py:23-57](file://backend/app/integrations/market_data/factory.py#L23-L57)
- [backend/app/domains/data/models.py:40-86](file://backend/app/domains/data/models.py#L40-L86)

## 架构总览
导入流程分为"命令行/接口触发—服务层—提供者—数据库"四层，支持 CSV、AKShare 和全市场同步三种来源，并通过工厂模式可扩展至 Tushare 等其他提供者。

```mermaid
sequenceDiagram
participant CLI as "命令行/客户端"
participant API as "API 层"
participant Svc as "导入服务"
participant FullSync as "全市场同步服务"
participant Prov as "提供者(本地/AKShare)"
participant Worker as "进程池工作器"
participant DB as "数据库"
CLI->>API : "POST /market-bars/import/*"
API->>Svc : "调用导入方法"
alt CSV 来源
Svc->>Prov : "读取CSV文件"
else AKShare 来源
Svc->>Prov : "并发抓取日线"
else 全市场同步
API->>FullSync : "触发全量同步"
FullSync->>Worker : "进程池并发获取"
Worker-->>FullSync : "返回批量数据"
end
Prov-->>Svc : "返回原始行集合"
FullSync->>DB : "批量插入/替换"
Svc->>DB : "查询并删除重复键(旧记录)"
Svc->>DB : "批量插入新记录"
DB-->>Svc : "提交成功"
Svc-->>API : "返回导入结果"
API-->>CLI : "导入摘要"
```

**图表来源**
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)
- [backend/app/services/market_data_import.py:185-241](file://backend/app/services/market_data_import.py#L185-L241)
- [backend/app/services/market_data_import.py:107-175](file://backend/app/services/market_data_import.py#L107-L175)
- [backend/app/services/market_data_full_sync.py:250-416](file://backend/app/services/market_data_full_sync.py#L250-L416)
- [backend/app/services/_market_fetch_worker.py:20-44](file://backend/app/services/_market_fetch_worker.py#L20-L44)
- [backend/app/domains/data/models.py:40-67](file://backend/app/domains/data/models.py#L40-L67)

## 详细组件分析

### 组件A：导入服务与提供者
- 导入服务职责
  - 接收原始行集合，执行标准化、去重、校验与 A 股标志计算。
  - 对重复键（symbol+trade_date）进行"先删后插"，保证幂等。
  - 返回导入结果对象，包含统计与错误明细。
- CSV 提供者
  - 读取 UTF-8 CSV，自动跳过空行，返回字典列表。
- AKShare 提供者
  - 异步并发遍历股票列表，每个股票在独立线程中调用 AKShare 接口。
  - 将返回的 DataFrame 转换为统一记录结构，并补全 symbol 字段。

```mermaid
classDiagram
class MarketDataImportService {
+import_csv_file(path, default_symbol, source_name)
+import_akshare(symbols, start_date, end_date, adjust)
+import_rows(rows, default_symbol, source)
-_normalize_rows(raw_rows, default_symbol)
-_replace_existing_rows(bars)
}
class LocalCsvDailyBarProvider {
+read(path) list[dict]
}
class AKShareDailyBarProvider {
+fetch(symbols, start_date, end_date, adjust) list[dict]
-_fetch_symbol(...)
}
MarketDataImportService --> LocalCsvDailyBarProvider : "使用"
MarketDataImportService --> AKShareDailyBarProvider : "使用"
```

**图表来源**
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_import.py:107-175](file://backend/app/services/market_data_import.py#L107-L175)

**章节来源**
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_import.py:107-175](file://backend/app/services/market_data_import.py#L107-L175)

### 组件B：全市场同步服务
- **新增** 全市场同步服务职责
  - 扫描全市场股票列表，计算增量同步起点。
  - 使用进程池并发获取数据，避免主线程阻塞。
  - 支持失败重试和ST标记更新。
  - 提供详细的进度跟踪和状态管理。
- **新增** 进程池工作器
  - 专门的模块用于进程池数据获取，避免启动时的依赖问题。
  - 隔离AKShare依赖，防止BrokenProcessPool错误。
- **新增** 进度跟踪
  - 支持扫描、同步、重试、ST标记更新等多个阶段。
  - 提供ETA估算、速率统计、错误计数等详细信息。

```mermaid
classDiagram
class MarketDataFullSyncService {
+start_full_sync(req) str
+_run(req) None
+_fetch_market_symbols() list[tuple]
+_existing_max_dates(symbols) dict[str, str]
+_persist_rows(rows) tuple[int, int]
+current_status() FullSyncProgress
}
class MarketFetchWorker {
+fetch_one_history(symbol, start_date, end_date, adjust) list[dict]
}
class FullSyncProgress {
+task_id : str | None
+phase : SyncPhase
+total : int
+done : int
+skipped : int
+inserted_rows : int
+updated_rows : int
+failures : int
+rate_per_sec : float
+eta_seconds : float | None
+last_symbol : str | None
+error : str | None
+is_running : bool
}
MarketDataFullSyncService --> MarketFetchWorker : "使用"
MarketDataFullSyncService --> FullSyncProgress : "管理"
```

**图表来源**
- [backend/app/services/market_data_full_sync.py:233-246](file://backend/app/services/market_data_full_sync.py#L233-L246)
- [backend/app/services/market_data_full_sync.py:250-416](file://backend/app/services/market_data_full_sync.py#L250-L416)
- [backend/app/services/_market_fetch_worker.py:20-44](file://backend/app/services/_market_fetch_worker.py#L20-L44)

**章节来源**
- [backend/app/services/market_data_full_sync.py:1-416](file://backend/app/services/market_data_full_sync.py#L1-L416)
- [backend/app/services/_market_fetch_worker.py:1-44](file://backend/app/services/_market_fetch_worker.py#L1-L44)

### 组件C：API 层导入接口
- CSV 导入接口
  - 请求体包含 CSV 路径与默认股票代码等参数。
  - 返回导入摘要，包含来源、总数、插入数、更新数、跳过数、时间范围、错误列表。
- AKShare 导入接口
  - 请求体包含股票列表、起止日期、复权方式等。
  - 返回与 CSV 导入一致的摘要结构。
- **新增** 全市场同步接口
  - POST `/market-bars/sync/full`：触发全量同步，支持并发度、板块过滤、增量模式等参数。
  - GET `/market-bars/sync/status`：轮询进度状态，返回详细的同步进度信息。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant API as "FastAPI路由"
participant FullSync as "全市场同步服务"
participant Worker as "进程池工作器"
participant DB as "数据库"
Client->>API : "POST /market-bars/sync/full"
API->>FullSync : "start_full_sync(req)"
FullSync->>Worker : "并发获取数据"
Worker-->>FullSync : "批量数据"
FullSync->>DB : "批量插入/替换"
DB-->>FullSync : "commit 成功"
FullSync-->>API : "FullSyncProgress"
API-->>Client : "MarketSyncStatus"
Client->>API : "GET /market-bars/sync/status"
API-->>Client : "当前进度状态"
```

**图表来源**
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)
- [backend/app/services/market_data_full_sync.py:233-246](file://backend/app/services/market_data_full_sync.py#L233-L246)
- [backend/app/services/market_data_full_sync.py:250-416](file://backend/app/services/market_data_full_sync.py#L250-L416)

**章节来源**
- [backend/app/api/v1/data.py:155-191](file://backend/app/api/v1/data.py#L155-L191)
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)

### 组件D：集成层提供者与工厂
- MarketDataProvider 抽象
  - 定义 get_history/get_snapshot/get_market_snapshot 等异步接口。
- 工厂选择
  - 支持 akshare/tushare/wind/mock，默认 akshare；未安装 akshare 时回退 mock。
- AKShareProvider
  - 实现历史/快照接口，内部以线程池执行阻塞调用，避免阻塞事件循环。
- MockProvider
  - 生成确定性合成行情，用于开发与测试。

```mermaid
classDiagram
class MarketDataProvider {
<<abstract>>
+get_history(symbol, start_date, end_date, freq, adjust)
+get_snapshot(symbols)
+get_market_snapshot()
+is_trading_hours()
}
class AKShareProvider {
+get_history(...)
+get_snapshot(...)
+get_market_snapshot()
+name = "akshare"
}
class MockMarketDataProvider {
+get_history(...)
+get_snapshot(...)
+get_market_snapshot()
+name = "mock"
}
MarketDataProvider <|.. AKShareProvider
MarketDataProvider <|.. MockMarketDataProvider
```

**图表来源**
- [backend/app/integrations/market_data/base.py:64-113](file://backend/app/integrations/market_data/base.py#L64-L113)
- [backend/app/integrations/market_data/akshare_provider.py:145-262](file://backend/app/integrations/market_data/akshare_provider.py#L145-L262)
- [backend/app/integrations/market_data/mock_provider.py:29-109](file://backend/app/integrations/market_data/mock_provider.py#L29-L109)

**章节来源**
- [backend/app/integrations/market_data/base.py:64-113](file://backend/app/integrations/market_data/base.py#L64-L113)
- [backend/app/integrations/market_data/factory.py:23-57](file://backend/app/integrations/market_data/factory.py#L23-L57)
- [backend/app/integrations/market_data/akshare_provider.py:145-262](file://backend/app/integrations/market_data/akshare_provider.py#L145-L262)
- [backend/app/integrations/market_data/mock_provider.py:29-109](file://backend/app/integrations/market_data/mock_provider.py#L29-L109)

### 组件E：数据模型与唯一约束
- MarketBarDaily
  - 唯一键：symbol + trade_date，确保同一天同一股票仅保留一条记录。
  - 字段覆盖开盘/最高/最低/收盘/成交量/成交额/复权价/涨跌停/ST/买卖限制等。
- **新增** StockInfo
  - 存储股票基本信息（中文名称、板块、ST标记、最后同步时间）。
  - 用于前端显示和符号搜索。
- 导入服务在持久化前会查询并删除重复键对应的旧记录，再批量插入新记录，从而实现幂等更新。

**章节来源**
- [backend/app/domains/data/models.py:40-86](file://backend/app/domains/data/models.py#L40-L86)
- [backend/app/services/market_data_import.py:270-289](file://backend/app/services/market_data_import.py#L270-L289)

### 组件F：命令行导入脚本
- 功能：支持从 CSV 或 AKShare 导入，输出 JSON 格式的导入结果。
- 参数：
  - CSV：路径、默认股票代码。
  - AKShare：股票列表、起止日期、复权方式。

**章节来源**
- [backend/scripts/import_market_data.py:14-53](file://backend/scripts/import_market_data.py#L14-L53)

## 依赖分析
- 组件耦合
  - 导入服务依赖 CSV/AKShare 提供者与数据库会话。
  - **新增** 全市场同步服务依赖进程池工作器、导入服务和数据库会话。
  - API 层依赖导入服务与数据库会话。
  - 集成层通过工厂解耦具体提供者，便于扩展。
- 外部依赖
  - AKShare：用于同步数据源的异步封装。
  - Pandas/Requests：用于数据转换与网络请求。
  - **新增** 进程池：用于隔离数据获取，避免BrokenProcessPool错误。
- 循环依赖
  - 未发现循环依赖；模块间单向依赖清晰。

```mermaid
graph LR
API["API 层(data.py)"] --> Svc["导入服务(market_data_import.py)"]
API --> FullSyncAPI["全市场同步API"]
Svc --> CSV["CSV提供者"]
Svc --> AK["AKShare提供者"]
FullSyncAPI --> FullSync["全市场同步服务"]
FullSync --> Worker["进程池工作器"]
FullSync --> ImportSvc["导入服务"]
FullSync --> DB["数据库模型(models.py)"]
Factory["工厂(factory.py)"] --> AKP["AKShareProvider"]
Factory --> Mock["MockProvider"]
```

**图表来源**
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_full_sync.py:233-246](file://backend/app/services/market_data_full_sync.py#L233-L246)
- [backend/app/services/_market_fetch_worker.py:20-44](file://backend/app/services/_market_fetch_worker.py#L20-L44)
- [backend/app/integrations/market_data/factory.py:23-57](file://backend/app/integrations/market_data/factory.py#L23-L57)
- [backend/app/integrations/market_data/akshare_provider.py:145-262](file://backend/app/integrations/market_data/akshare_provider.py#L145-L262)
- [backend/app/integrations/market_data/mock_provider.py:29-109](file://backend/app/integrations/market_data/mock_provider.py#L29-L109)
- [backend/app/domains/data/models.py:40-86](file://backend/app/domains/data/models.py#L40-L86)

**章节来源**
- [backend/app/api/v1/data.py:155-191](file://backend/app/api/v1/data.py#L155-L191)
- [backend/app/services/market_data_import.py:177-290](file://backend/app/services/market_data_import.py#L177-L290)
- [backend/app/services/market_data_full_sync.py:1-416](file://backend/app/services/market_data_full_sync.py#L1-L416)
- [backend/app/integrations/market_data/factory.py:23-57](file://backend/app/integrations/market_data/factory.py#L23-L57)

## 性能考虑
- 异步与并发
  - AKShare 提供者使用线程池执行阻塞调用，避免阻塞事件循环。
  - 导入服务对多个股票并发抓取，提升吞吐。
  - **新增** 全市场同步服务使用进程池并发获取数据，避免主线程阻塞。
- 批量处理
  - 先在内存中完成标准化、去重、校验，再一次性批量插入/替换，减少数据库往返。
  - **新增** 进程池工作器批量处理数据，提高整体效率。
- 唯一键约束
  - 使用唯一索引（symbol+trade_date）保障幂等，替换旧记录时先删除再插入，避免重复写入。
  - **新增** StockInfo使用ON CONFLICT DO UPDATE，支持高效的信息更新。
- 解析与校验
  - 字段别名映射与宽松解析降低数据格式差异带来的失败率。
  - 数值解析与正则/边界校验防止脏数据入库。
- **新增** 进程池隔离
  - 专门的工作器模块避免启动时的依赖问题，防止BrokenProcessPool错误。
  - 每个worker运行N次后重启，防止mini_racer/V8累积状态污染。

## 故障排查指南
- 常见错误类型
  - 缺失必要字段：如 symbol/date/open/high/low/close/volume。
  - 数值非法：空值、负值、非有限浮点数。
  - 日期格式错误：无法解析或为空。
  - 重复键：同 symbol+trade_date 的重复记录被计为跳过。
  - 未安装 AKShare：提示改用本地 CSV 导入。
  - **新增** 进程池错误：BrokenProcessPool、worker重启等。
- API 层错误处理
  - 文件不存在或导入错误会被包装为 HTTP 404/400 并返回错误信息。
  - **新增** 全市场同步错误：AlreadyRunningError、同步失败等。
- 单元测试参考
  - 验证去重、替换旧记录、A 股标志计算、错误计数与时间范围统计。
  - **新增** 全市场同步：并发处理、进度跟踪、ST标记更新。

**章节来源**
- [backend/app/services/market_data_import.py:25-105](file://backend/app/services/market_data_import.py#L25-L105)
- [backend/app/services/market_data_import.py:389-403](file://backend/app/services/market_data_import.py#L389-L403)
- [backend/app/api/v1/data.py:168-172](file://backend/app/api/v1/data.py#L168-L172)
- [backend/app/services/market_data_full_sync.py:225-231](file://backend/app/services/market_data_full_sync.py#L225-L231)
- [backend/tests/services/test_market_data_import.py:25-134](file://backend/tests/services/test_market_data_import.py#L25-L134)

## 结论
该导入体系以服务层为核心，结合提供者抽象与工厂模式，实现了 CSV、AKShare 和全市场同步的灵活导入，并通过唯一约束与批量替换保障幂等与性能。**新增的全市场同步服务**提供了完整的A股市场数据同步能力，支持进程池并发处理、详细的进度跟踪和ST标记更新。API 层与脚本入口提供了统一的调用方式，测试覆盖了关键行为。后续可通过工厂扩展 Tushare 等提供者，或在适配器层补充 Tushare 的具体实现。

## 附录

### 数据导入服务处理流程（算法流程）
```mermaid
flowchart TD
Start(["开始"]) --> ReadRows["读取原始行"]
ReadRows --> Normalize["标准化/别名映射/类型解析"]
Normalize --> Validate["字段与数值校验"]
Validate --> Valid{"有效?"}
Valid --> |否| RecordErr["记录错误并跳过"]
Valid --> |是| Dedup["按(symbol, trade_date)去重"]
Dedup --> Flags["计算A股标志(涨停/跌停/停牌/买卖限制)"]
Flags --> ReplaceOld["查询并删除重复键旧记录"]
ReplaceOld --> Persist["批量持久化新记录"]
Persist --> Summary["汇总统计(总数/导入数/插入数/更新数/跳过数/错误)"]
Summary --> End(["结束"])
RecordErr --> Summary
```

**图表来源**
- [backend/app/services/market_data_import.py:213-241](file://backend/app/services/market_data_import.py#L213-L241)
- [backend/app/services/market_data_import.py:243-268](file://backend/app/services/market_data_import.py#L243-L268)
- [backend/app/services/market_data_import.py:270-289](file://backend/app/services/market_data_import.py#L270-L289)
- [backend/app/services/market_data_import.py:405-422](file://backend/app/services/market_data_import.py#L405-L422)

### 全市场同步服务处理流程（算法流程）
```mermaid
flowchart TD
Start(["开始全量同步"]) --> Scan["扫描全市场股票"]
Scan --> Filter["板块过滤"]
Filter --> CalcStart["计算增量起点"]
CalcStart --> Plan["生成同步计划"]
Plan --> BatchLoop["批次循环"]
BatchLoop --> Fetch["进程池并发获取"]
Fetch --> Merge["合并批量数据"]
Merge --> Persist["批量持久化"]
Persist --> Retry["失败重试"]
Retry --> UpdateST["更新ST标记"]
UpdateST --> Complete["完成"]
Complete --> End(["结束"])
```

**图表来源**
- [backend/app/services/market_data_full_sync.py:250-416](file://backend/app/services/market_data_full_sync.py#L250-L416)
- [backend/app/services/_market_fetch_worker.py:20-44](file://backend/app/services/_market_fetch_worker.py#L20-L44)

### 导入脚本示例（命令行）
- CSV 导入
  - 用法：python scripts/import_market_data.py csv <path> [--default-symbol <code>]
- AKShare 导入
  - 用法：python scripts/import_market_data.py akshare --symbols <code...> --start-date <YYYY-MM-DD> --end-date <YYYY-MM-DD> [--adjust qfq|hfq]
- **新增** 全市场同步
  - 用法：python -m backend.app.services.market_data_full_sync

**章节来源**
- [backend/scripts/import_market_data.py:14-53](file://backend/scripts/import_market_data.py#L14-L53)

### 数据格式要求与字段别名
- 必填字段：symbol、trade_date、open、high、low、close、volume
- 可选字段：amount、adjusted_close、forward_factor、is_st
- 字段别名（示例）：symbol 包含 symbol/code/ticker/sec_code/股票代码/代码；date/time/交易日期等映射到 trade_date
- 日期格式：支持 ISO、带斜杠/点号、8 位数字（YYYYMMDD）等常见格式，自动规范化为 YYYY-MM-DD
- 数值规则：open/high/low/close/amount/adjusted_close 必须为有限正值；volume 非负；is_st 支持多种真值表示

**章节来源**
- [backend/app/services/market_data_import.py:90-105](file://backend/app/services/market_data_import.py#L90-L105)
- [backend/app/services/market_data_import.py:326-387](file://backend/app/services/market_data_import.py#L326-L387)

### 导入进度跟踪与异常恢复
- 进度跟踪
  - 导入结果包含 total_rows/imported_rows/skipped_rows/symbols/start_date/end_date/errors 等字段，可用于前端展示与审计。
  - **新增** 全市场同步进度包含task_id、phase、total、done、skipped、inserted_rows、updated_rows、failures、rate_per_sec、eta_seconds、last_symbol、error、is_running等详细信息。
- 异常恢复
  - 导入服务在遇到无效行时记录错误并继续处理其余行，最终返回错误列表。
  - 重复键采用"替换旧记录"的策略，确保最终状态与输入一致。
  - API 层捕获导入异常并返回标准 HTTP 错误码。
  - **新增** 全市场同步支持失败重试和进程池重启机制。

**章节来源**
- [backend/app/services/market_data_import.py:75-88](file://backend/app/services/market_data_import.py#L75-L88)
- [backend/app/services/market_data_import.py:25-105](file://backend/app/services/market_data_import.py#L25-L105)
- [backend/app/api/v1/data.py:168-172](file://backend/app/api/v1/data.py#L168-L172)
- [backend/app/services/market_data_full_sync.py:365-416](file://backend/app/services/market_data_full_sync.py#L365-L416)

### 全市场同步API接口规范
- 触发全量同步：POST `/data/market-bars/sync/full`
  - 参数：startDate、endDate、adjust、concurrency、boards、incremental
  - 返回：MarketSyncStatus（任务ID、阶段、进度、统计信息）
- 查询进度：GET `/data/market-bars/sync/status`
  - 返回：当前同步状态，支持轮询

**章节来源**
- [backend/app/api/v1/data.py:430-453](file://backend/app/api/v1/data.py#L430-L453)
- [backend/app/domains/data/schemas.py:178-211](file://backend/app/domains/data/schemas.py#L178-L211)