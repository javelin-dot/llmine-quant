...
## 简介
本文件面向 llmine-quant 后端的数据管理子系统，聚焦于四大核心数据域：MarketBarDaily（日线时间序列）、DataSource（数据源配置）、FeatureSet（特征注册）与 Agent/Workflow（智能体编排）。文档从数据库模式设计、表间关系、索引策略、Alembic 迁移管理、版本控制与数据迁移策略、查询优化、分区与备份恢复等方面进行系统化阐述，并提供可操作的实践建议。

## 项目结构
围绕数据域的关键文件组织如下：
- 模型定义：位于数据域和智能体域模型文件中，定义了 MarketBarDaily、DataSource、FeatureSet、AgentDefinition、WorkflowDefinition、WorkflowNode、WorkflowEdge、WorkflowVersion 及其关联表等。
...
- 服务与 API：market_data_import.py 实现 CSV/AKShare 导入与规范化；agents.py 提供智能体编排与工作流发布接口。
...
subgraph "智能体域模型"
AD["AgentDefinition<br/>智能体定义"]
WD["WorkflowDefinition<br/>工作流定义"]
WN["WorkflowNode<br/>工作流节点"]
WE["WorkflowEdge<br/>工作流边"]
WV["WorkflowVersion<br/>工作流版本"]
end
subgraph "基类与会话"
...
V3["迁移脚本<br/>workflow_versions.py"]
V4["迁移脚本<br/>stock_info.py"]
end
subgraph "服务与API"
IMP["MarketDataImportService<br/>导入服务"]
API["API 路由 data.py"]
AGENTAPI["API 路由 agents.py"]
END
MBD --> BC
DS --> BC
FS --> BC
...
- [backend/app/db/migrations/versions/20260516_000001_workflow_versions.py:36-117](file://backend/app/db/migrations/versions/20260516_000001_workflow_versions.py#L36-L117)
...
## 核心组件
本节对四个核心表域进行深入解析，涵盖字段语义、约束与索引策略。
...
- AgentDefinition（智能体定义）
  - 设计理念：可重用的、完全可配置的智能体模板，支持角色分类、自主性配置和完整的输入输出模式。
  - 关键字段：name、role、avatar、description、objective、downstream_hint、autonomy、status、model_config_json、system_prompt、user_prompt_template、input_schema_json、output_schema_json、normalized_input_schema_json、normalized_output_schema_json、input_mapping_json、output_mapping_json、tool_policy_json、constraints_json、runtime_policy_json 等。
  - 约束与索引：name 唯一；role 字段建立索引；JSON 字段存储复杂配置和模式定义。

- WorkflowDefinition（工作流定义）
  - 设计理念：保存多个智能体定义组成的工作流，支持版本管理和发布状态控制。
  - 关键字段：name、description、version、status、is_default、published_version_id 等。
  - 约束与索引：name 唯一；published_version_id 建立索引以支持发布版本查找。

- WorkflowNode（工作流节点）
  - 设计理念：放置在工作流画布上的智能体实例，支持位置信息和配置覆盖。
  - 关键字段：workflow_id、agent_definition_id、label、position_x、position_y、config_override_json 等。
  - 约束与索引：workflow_id 和 agent_definition_id 建立索引以支持快速查找。

- WorkflowEdge（工作流边）
  - 设计理念：两个工作流节点之间的有向连接，支持数据映射和条件配置。
  - 关键字段：workflow_id、source_node_id、target_node_id、mapping_json、condition_json 等。
  - 约束与索引：所有关键字段都建立索引以支持高效的图遍历。

- WorkflowVersion（工作流版本）
  - 设计理念：工作流图和智能体定义的不可变发布快照，支持版本控制和回溯分析。
  - 关键字段：workflow_id、version、status、snapshot_json 等。
  - 约束与索引：workflow_id 建立索引以支持版本查询。
...
- [backend/app/domains/agents/models.py:64-88](file://backend/app/domains/agents/models.py#L64-L88)
- [backend/app/domains/agents/models.py:91-101](file://backend/app/domains/agents/models.py#L91-L101)
- [backend/app/domains/agents/models.py:115-126](file://backend/app/domains/agents/models.py#L115-L126)
- [backend/app/domains/agents/models.py:128-137](file://backend/app/domains/agents/models.py#L128-L137)
- [backend/app/domains/agents/models.py:104-113](file://backend/app/domains/agents/models.py#L104-L113)
...
participant DataAPI as "数据API 路由 data.py"
participant AgentAPI as "智能体API 路由 agents.py"
participant Service as "MarketDataImportService"
participant AgentService as "AgentOrchestrator"
participant ORM as "SQLAlchemy ORM"
participant DB as "PostgreSQL/ TimescaleDB"
Client->>DataAPI : "POST /market-bars/import/csv 或 /market-bars/import/akshare"
DataAPI->>Service : "构造导入服务并调用导入方法"
Service->>Service : "规范化与去重处理"
Service->>ORM : "批量写入 MarketBarDaily"
ORM->>DB : "INSERT/UPDATE 执行"
DB-->>ORM : "返回受影响行数"
ORM-->>Service : "提交事务"
Service-->>DataAPI : "返回导入结果"
DataAPI-->>Client : "返回导入摘要"
Client->>AgentAPI : "POST /agents/workflows/{id}/publish"
AgentAPI->>AgentService : "工作流发布服务"
AgentService->>AgentService : "验证合约完整性"
AgentService->>ORM : "创建不可变版本快照"
ORM->>DB : "INSERT WorkflowVersion"
DB-->>ORM : "返回版本ID"
ORM-->>AgentService : "提交事务"
AgentService-->>AgentAPI : "返回发布结果"
AgentAPI-->>Client : "返回工作流状态"
```

**图表来源**
- [backend/app/api/v1/data.py:155-191](file://backend/app/api/v1/data.py#L155-L191)
- [backend/app/api/v1/agents.py:173-207](file://backend/app/api/v1/agents.py#L173-L207)
...
### AgentDefinition 工作流编排设计
- AgentDefinition：可重用的智能体模板，支持完整的输入输出模式定义、工具策略和运行时配置。
- WorkflowDefinition：保存多个智能体定义组成的工作流，支持版本管理和发布状态控制。
- WorkflowNode：工作流画布上的智能体实例，支持位置信息和配置覆盖。
- WorkflowEdge：两个工作流节点之间的有向连接，支持数据映射和条件配置。
- WorkflowVersion：工作流图和智能体定义的不可变发布快照，支持版本控制和回溯分析。
- 索引策略：
  - agent_definitions.name：按智能体名称检索。
  - workflow_definitions.name：按工作流名称检索。
  - workflow_nodes.workflow_id：按工作流查找节点。
  - workflow_edges.workflow_id：按工作流查找边。
  - workflow_versions.workflow_id：按工作流查询版本。
...
+string autonomy
+string status
+string model_config_json
+string system_prompt
+string user_prompt_template
+string input_schema_json
+string output_schema_json
+string normalized_input_schema_json
+string normalized_output_schema_json
+string input_mapping_json
+string output_mapping_json
+string tool_policy_json
+string constraints_json
+string runtime_policy_json
}
...
+string id
+string workflow_id
+string agent_definition_id
+string label
+float position_x
+float position_y
+string config_override_json
}
class WorkflowEdge {
+string id
+string workflow_id
...
AgentDefinition --> WorkflowNode : "被引用"
WorkflowDefinition --> WorkflowNode : "包含"
WorkflowDefinition --> WorkflowEdge : "包含"
WorkflowNode --> WorkflowEdge : "连接"
```
...
### 工作流发布与版本控制机制
- 不可变快照：WorkflowVersion 存储完整的工作流图和智能体定义快照，确保历史版本的可追溯性。
- 发布流程：工作流发布前进行合约完整性验证，包括输入输出模式匹配、数据映射验证等。
- 版本演进：支持语义化版本管理，发布后的新修改不影响已发布的版本。
- 合约验证：自动检查上游智能体的输出是否满足下游智能体的输入要求。

**章节来源**
- [backend/app/api/v1/agents.py:232-260](file://backend/app/api/v1/agents.py#L232-L260)
...
- 工作流依赖：AgentDefinition 作为 WorkflowNode 的外键依赖，WorkflowDefinition 作为 WorkflowNode 和 WorkflowEdge 的外键依赖，形成完整的智能体编排体系。
...
BC --> LN["LineageNode"]
BC --> LE["LineageEdge"]
BC --> AD["AgentDefinition"]
BC --> WD["WorkflowDefinition"]
BC --> WN["WorkflowNode"]
BC --> WE["WorkflowEdge"]
BC --> WV["WorkflowVersion"]
ENV["Alembic 环境<br/>env.py"] --> MIG["迁移脚本"]
S["AsyncSessionLocal"] --> API["API 层"]
API --> SVC["导入服务"]
API --> AGENTSVC["AgentOrchestrator"]
...
- 查询优化建议
  ...
  - 工作流查询：按 workflow_id 查询节点和边时，利用索引字段进行高效图遍历。
- 分区策略
...
- 工作流发布
  - 合约验证失败：检查上游智能体的 normalized_output_schema_json 是否与下游智能体的 normalized_input_schema_json 匹配。
  - 版本冲突：确认工作流版本号的语义化管理，避免重复发布相同版本。
  - 图遍历错误：验证 WorkflowNode 和 WorkflowEdge 的外键关系，确保图结构的完整性。
...
## 结论
本数据库模式围绕"时间序列（MarketBarDaily）、数据源（DataSource）、特征与血缘（FeatureSet）、智能体编排（AgentDefinition/Workflow）"四轴展开，通过统一的 BaseModel 审计字段与 Alembic 迁移体系，实现了清晰的版本演进与跨模块协作。新增的工作流编排功能支持复杂的智能体协作场景，通过不可变版本快照确保历史版本的可追溯性和系统的稳定性。索引策略与查询优化建议有助于在海量数据场景下保持高性能与可维护性。结合 TimescaleDB 分区与物化视图，可进一步提升时间序列分析与回测查询效率。
